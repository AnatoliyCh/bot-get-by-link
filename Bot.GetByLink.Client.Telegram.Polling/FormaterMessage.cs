...
public class FormaterMessage
{
    ...
    private static (string MutableUrlText, string[] MutableUrlPicture, string[] MutableUrlVideo) GetTextUrlAndValidUrl(
        string[] urlPicture, string[] urlVideo)
    {
        ...
        {
            var result = GetSizeContentUrl(client, inputMediaPhoto);
        ...
        {
            var result = GetSizeContentUrl(client, inputMediaVideo);
    ...
    private static long GetSizeContentUrl(HttpClient client, string url)
    {
        ...
        if (long.TryParse(
                ...
                out var contentLength)) result = contentLength == 0 ? contentLength : contentLength / (1024 * 1024);
    ...
    private static List<IAlbumInputMedia> GetPhotoInputMedia(string[] urlPictures, string captionMedia)
    {
        ...
        if (urlPictures.Length > 0)
        {
            ...
            foreach (var inputMediaPhoto in urlPictures) listInputMediaPhoto.Add(new InputMediaPhoto(inputMediaPhoto));
    ...
    private static List<IAlbumInputMedia> GetVideoInputMedia(string[] urlVideo, string captionMedia, bool isFirstMedia)
    {
        ...
        if (urlVideo.Length > 0)
        {
            ...
            foreach (var inputMediaVideo in urlVideo) listInputMediaVideo.Add(new InputMediaVideo(inputMediaVideo));
    ...
    private static List<string> GetMessageArray(string textPost, int captionLength)
    {
        ...
        if (textPost.Length > captionLength)
        {
            ...
            for (var i = 0; textPost.Length > 4096; i++)
            {
