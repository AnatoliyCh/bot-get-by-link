...
namespace Bot.GetByLink.Client.Telegram.Common.Model
{
    ...
    /// </summary>
    public class BuilderMessage : IBuilderMessage
    {
        /// <summary>
        ...
        private IEnumerable<IEnumerable<IAlbumInputMedia>> GetListInputMedia(IEnumerable<IMediaInfo> listMedia, IEnumerable<string> textMessages)
        {
            var listFormatedMedias = listMedia.Select<IMediaInfo, InputMediaBase>((media, index) =>
            {
                return media.Type == GetByLink.Common.Infrastructure.Enums.MediaType.Photo ? new InputMediaPhoto(media.Url) : new InputMediaVideo(media.Url);
            });

            var groupListFormatedMedias = listFormatedMedias.Split(MaxColMediaInMessage).Select(x => x.ToList()).ToList();

            for (int i = 0; i < groupListFormatedMedias.Count; i++)
            {
        ...
        {
            if ((countMediaMessage == 0 && allText.Length < MaxTextLenghtMessage) || (countMediaMessage > 0 && allText.Length < MaxTextLenghtFirstMedia)) return new List<string>() { allText };
            ...
            while (finderTextMessage.Length > 0)
            {
                ...
                var sizeCurrentMessage = finderTextMessage.Length > maxSizeMessage ? maxSizeMessage : finderTextMessage.Length;
                ...
                var intedParagraph = intedParagraphEndSentecis == -1 ? textMessage.LastIndexOf("\n") : intedParagraphEndSentecis;
